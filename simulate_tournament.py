...
def simulate_group(model, df, group_name, teams):
    ...
    for i in range(len(teams)):
        for j in range(i + 1, len(teams)):
            ...
            _, _, points_a, points_b = simulate_group_match(model, df, team_a, team_b)
...
def main():
    ...
    for group_name, teams in GROUPS.items():
        ...
        for row in standings:
            print(
                f"{row['team']}: {row['points']} pts, "
                f"{row['wins']}W-{row['draws']}D-{row['losses']}L"
            )

        top_two = standings[:2]
        all_advancing.extend([team["team"] for team in top_two])

    print("\nTeams advancing:")
    for team in all_advancing:
        print(team)
